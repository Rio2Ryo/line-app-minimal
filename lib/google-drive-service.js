...
class GoogleDriveService {
  ...
  // 簡素化されたフォルダ作成（メインフォルダに直接保存）
  async createSimpleFolder(sourceInfo, date = new Date()) {
    // 日本時間に変換（JST = UTC+9）
    const jstDate = new Date(date.getTime() + (9 * 60 * 60 * 1000));
    const dateStr = jstDate.toISOString().split('T')[0]; // YYYY-MM-DD
    
    console.log(`📅 簡素化フォルダ作成: 日付=${dateStr}`);
    ...
    try {
      ...
      console.log(`📁 メインフォルダに直接保存: ${groupIdentifier}_${dateStr}`);

      // メインフォルダの情報を返す（サブフォルダは作成しない）
      return {
        dateFolder: {
          id: this.config.google.driveFolderId,
          name: 'LINE_Messages'
        },
        groupIdentifier,
        dateStr
      ...
      console.error('❌ 簡素化フォルダ作成エラー:', error);
      throw error;
    }
  }
  ...
  async saveTextMessage(messageText, sourceInfo, timestamp) {
    try {
      console.log(`💾 テキストメッセージ保存開始: ${messageText.substring(0, 50)}...`);
      
      const { dateFolder, groupIdentifier, dateStr } = await this.createSimpleFolder(sourceInfo, new Date(timestamp));
      ...
      const fileName = `${groupIdentifier}_${dateStr}_messages.txt`;
      
      console.log(`📝 テキストファイルとして保存: ${fileName}`);
      
      // メッセージを整形（IATO形式: いつ・誰が・何を）
      const messageEntry = `【${timeStr}】 ${sourceInfo.userId || 'Unknown User'}\n${messageText}\n\n`;
      
      console.log(`📝 保存するメッセージ: ${messageEntry.substring(0, 100)}...`);
      ...
      try {
        ...
        console.log(`✅ テキストファイル作成完了: ${fileName} (ID: ${fileResponse.data.id})`);
