...
async function saveToGoogleDrive(messageText, sourceInfo, timestamp, config) {
  try {
    ...
    const timeStr = originalDate.toLocaleString('ja-JP', { 
      timeZone: 'Asia/Tokyo',
      year: 'numeric',
      month: '2-digit',
      day: '2-digit',
    ...
    // 日付フォルダ用のJST日付を取得
    const jstDateStr = originalDate.toLocaleDateString('ja-JP', { timeZone: 'Asia/Tokyo' });
    const [year, month, day] = jstDateStr.split('/').map(num => num.padStart(2, '0'));
    const jstDateForFolder = new Date(`${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`);
    
    console.log(`🕒 時刻変換: ${timestamp} → JST: ${timeStr} → フォルダ日付: ${year}-${month}-${day}`);
    
    // 日付フォルダを取得または作成（グループフォルダ内）
    const dateFolder = await getOrCreateDateFolder(drive, groupFolder.folderId, jstDateForFolder);
    
    // 日次テキストファイルを取得または作成
    const dailyTextFile = await getOrCreateDailyTextFile(drive, dateFolder.folderId, jstDateForFolder);
...
async function saveFileToGoogleDrive(messageId, fileName, sourceInfo, timestamp, config) {
  try {
    ...
    console.log(`🕒 ファイル時刻変換: ${timestamp} → JST: ${timeStr} → フォルダ日付: ${year}-${month}-${day}`);
    ...
    const savedFileName = `${year}-${month}-${day}_${Date.now()}_${originalFileName}`;
